...
// return latest stream, for test purpose
app.get('/latest', (req, res) => {
  res.statusCode = 200;
  res.setHeader('Content-Type', 'application/json');

  db.sublevel('stream').createReadStream({
    values: false,
    limit: 1,
    reverse: true
  })
  .pipe(through.obj(function(chunk, _, cb) {
    this.push(json(chunk));
    cb();
  }))
  .pipe(res);
});
...
app.get('/:id', (req, res) => {
  db.sublevel('stream').get(req.params.id, (err, data) => {
...
app.post('/', (req, res) => {
  ...
  db.sublevel('stream').put(key, req.body, () => {
